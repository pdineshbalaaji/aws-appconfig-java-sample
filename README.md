...
The `.github/workflows/aws-transform.yml` file provides:
...
**Usage:**
1. Push code to branches matching `aws-transform-*` pattern
...
6. **aws-transform**: Generates dependencies for AWS Transform (pre-transformation only)
...
1. **For GitHub**: Push to a branch matching `aws-transform-*`
2. **For GitLab**: Push to any branch (regular pipeline) or `atx/transform-*` (with AWS Transform)
